		...
		if(rdata.DISK != undefined){
			...
			if(rdata.FILES != null && rdata.FILES !=''){
				for(var i=0; i < rdata.FILES.length; i++){
					rdata.FILES[i] = rdata.FILES[i].split(";")[0];
...
function OnlineEditFile(type, fileName) {
	if (type == 1) {
		...
		$.post('/files.php?action=SaveFileBody', 'data=' + data + '&file=' + fileName, function(rdata) {
			layer.closeAll();
			layer.msg(rdata.msg, {
	...
	$.post('/files.php?action=GetFileBody', 'file=' + fileName, function(rdata) {
		...
		layer.open({
			...
			<p style="color:red;margin-bottom:10px">提示：Ctrl+F 搜索关键字，Ctrl+G 查找下一个，Ctrl+S 保存，Ctrl+Shift+R 查找替换!</p>\
