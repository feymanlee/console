  ...
  $.get(sUrl, function (data) {
    ...
    for (var i = 0; i < data.data.length; i++) {
      ...
      Body += "<tr><td style='display:none'><input type='checkbox' name='id' value='" + data.data[i].id + "'></td>\
					<td><a class='link webtips' href='javascript:;' onclick=\"DomainRoot(" + data.data[i].id + ",'" + data.data[i].name + "')\">" + data.data[i].name + " (" + domain.length + ")</td>\
					<td>" + status + "</td>\
					<td>" + backup + "</td>\
  ...
  $.get("/Ajax.php?action=getKey&tab=sites&key=path&id=" + id, function (rdata) {
    $.get('/site.php?action=GetDirUserINI&path=' + rdata, function (userini) {
      ...
      var webPathHtml = "<div class='webEdit-box padding-10'>\
						<div style='margin-left: 80px;'><input type='checkbox' name='userini' id='userini'" + checkeds + " /><label for='userini' style='font-weight:normal'>防跨站攻击</label></div>\
						<div class='line'>\
						<label><span>根目录</span></label>\
						<div class='info-r'>\
							...
							<br><br><button class='btn btn-success btn-sm' onclick='SetSitePath(" + id + ")'>保存</button>\
						</div>\
					</div>";
  ...
  $.get('/site.php?action=GetIndex&id=' + id, function (rdata) {
    ...
    var setIndexHtml = "<div class='webEdit-box padding-10'><form id='SetIndex'><div class='SetIndex'>\
				<div class='line'>\
					...
					<div class='info-r'>\
						<textarea id='Dindex' name='files' style='margin-top: 2px; margin-bottom: 0px; height: 186px; width:60%; line-height:20px'>" + rdata + "</textarea>\
						<p style='line-height: 26px; color: #666'>默认文档，每行一个，优先级由上至下。</p>\
						</br><button type='button' class='btn btn-success btn-sm' onclick='SetIndexList(" + id + ")'>确定</button>\
					</div>\
				</div>\
				</div></form></div>";
...
function webStop(wid, wname) {
  layer.confirm('站点停用后将无法访问，您真的要停用这个站点吗？', function (index) {
    if (index > 0) {
      var loadT = layer.load()
...
function webStart(wid, wname) {
  layer.confirm('即将启动站点，您真的要启用这个站点吗？', function (index) {
    if (index > 0) {
      var loadT = layer.load()
...
function checkDomainWebsize(obj, domain) {
  ...
  $.ajax({
    url: gurl, data: data, type: "get", dataType: "jsonp", async: true, success: function (rdata) {
...
function delDomain(wid, wname, domain, port, type) {
  ...
  layer.confirm('您真的要从站点中删除这个域名吗？', function (index) {
...
function backupDel(bid, path, wname, file) {
  layer.confirm('删除打包文件将无法恢复，您真的要删除这份打包文件吗？', function (index) {
    if (index > 0) {
...
function WebBackupDelete(id, pid) {
  layer.confirm('真的要删除备份包吗?', {title: '删除备份文件'}, function (index) {
