	...
	$.get(sUrl, function(data) {
		...
		for (var i = 0; i < data.data.length; i++) {
			...
			Body += "<tr><td><input type='checkbox' name='id' value='"+data.data[i].id+"'>\
					...
					<td><span class='password' data-pw='"+data.data[i].password+"'>**********</span><span class='glyphicon glyphicon-eye-open cursor pw-ico' style='margin-left:10px'></span></td>\
					...
					<a href='javascript:;' class='link' onclick=\"AdminDatabase('"+data.data[i].name+"','"+data.data[i].username+"','"+data.data[i].password+"')\" title='管理数据库'>管理</a> | \
					<a href='javascript:;' class='link' onclick=\"SetDatabaseAccess('"+data.data[i].name+"')\" title='设置访问权限'>权限</a> | \
					<a href='javascript:;' class='link' onclick=\"DataRespwd(0,'"+data.data[i].id+"','"+data.data[i].username+"')\" title='修改数据库密码'>改密</a> | \
...
function DataAdd(sign){
	if(sign==0){
		...
		content: "<form class='zun-form-new' id='DataAdd'>\
						<div class='line'>\
						<label><span>数据库名</span></label><div class='info-r'><input  type='text' name='name' placeholder='新的数据库名' /></div>\
						</div>\
						<div class='line'>\
						...
						<label><span>备注</span></label><div class='info-r'><input type='text' name='bak' placeholder='数据库备注' /></div>\
						</div>\
		...
		if (sign == 0) {
			$.get('/Ajax.php?action=getKey&tab=config&key=mysql_root&id=1',function(rdata){
				...
				content: "<form class='zun-form-new'' id='DataSetuppwd'>\
						<div class='line'>\
						<label style='width:14%'><span>root密码:</span></label><div class='info-r'><input id='MyPassword' type='text' name='password' value='"+mypasswd+"' style='width:350px' /><span title='随机密码' class='glyphicon glyphicon-repeat cursor' onclick='repeatPwd(24)'></span>\
						</div></div>\
				        <div class='submit-btn'>\
							<button type='button' class='btn btn-danger btn-sm btn-title' onclick='layer.closeAll()'>取消</button>\
					        <button type='button' class='btn btn-success btn-sm btn-title' onclick='DataSetuppwd(1)' >提交</button>\
				        </div>\
				      </form>"
			});
			RandomStrPwd(24);
...
function DataRespwd(sign,id,username){
	...
	layer.confirm("您确定要修改数据库的密码吗?",{title:'数据库管理'},function(index){
			if(index>0){
				var loadT=layer.load({shade:true,shadeClose:false});
...
function DataDetails(id,dataname){
	$.get('/Ajax.php?action=getFind&tab=databases&id='+id,function(rdata){
		...
		for(var i=0;i<frdata.data.length;i++){
			...
			var ftpdown = " | <a class='link' href='/files.php?action=GetFileBytes&file="+frdata.data[i].filename+"' target='_blank'>下载</a>";
			 body += "<tr><td><span class='glyphicon glyphicon-file'></span>"+frdata.data[i].name+"</td>\
							...
							<td style='color:#bbb;text-align:right'><a class='link' herf='javascrpit:;' onclick=\"RecoveryData('"+frdata.data[i].filename+"','"+dataname+"')\">恢复</a>\
...
function RecoveryData(fileName,dataName){
	layer.confirm("数据库将被覆盖,继续吗?",{title:'导入数据'},function(index){
		var loadT =layer.msg('正在导入，请稍候...', {icon: 16,time:0});
...
function DataBackup(id,dataname){
	var loadT =layer.msg('正在备份，请稍候...', {icon: 16,time:0});
...
function DataBackupDelete(typeid,id,dataname){
	layer.confirm("真的要删除备份文件吗?",{title:'删除备份'},function(index){
		var loadT=layer.load({shade:true,shadeClose:false});
