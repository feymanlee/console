...
class PMA_DbQbe
{
    ...
    public function __construct(
        $dbname, $savedSearchList = [], $currentSearch = null
    ) {
        $this->_db              = $dbname;
    ...
    private function _getOrderByClause()
    {
        ...
        for (
            ...
            if (/*overload*/
                mb_substr($this->_curField[$column_index], -2) == '.*'
            ) {
                continue;
            }
    ...
    private function _getWhereClauseTablesAndColumns()
    {
        ...
        for (
            ...
            if (!empty($this->_criteria[$column_index])) {
                if (/*overload*/
                    mb_substr($this->_criteria[$column_index], 0, 1) == '='
                    || /*$pmaString->*/
    ...
    public function getSelectionForm()
    {
        ...
        $html_output .= PMA_URL_getHiddenInputs(['db' => $this->_db]);
        // get SQL query
        $html_output .= '<div class="floatleft">';
        $html_output .= '<fieldset>';
        $html_output .= '<legend>'
