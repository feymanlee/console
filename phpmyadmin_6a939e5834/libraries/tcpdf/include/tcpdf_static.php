...
class TCPDF_STATIC
{
    ...
    /**
     * Sets the current active configuration setting of magic_quotes_runtime (if the set_magic_quotes_runtime function
     * exist)
     *
    ...
    /**
     * Gets the current active configuration setting of magic_quotes_runtime (if the get_magic_quotes_runtime function
     * exist)
     ...
     * @param $type    (string) valid values are: <ul><li>'MediaBox' : the boundaries of the physical medium on which
     *                 the page shall be displayed or printed;</li><li>'CropBox' : the visible region of default user
     *                 space;</li><li>'BleedBox' : the region to which the contents of the page shall be clipped when
     *                 output in a production environment;</li><li>'TrimBox' : the intended dimensions of the finished
     *                 page after trimming;</li><li>'ArtBox' : the page's meaningful content (including potential white
     *                 space).</li></ul>
     ...
     *
     * @param $layout (string) The page layout. Possible values are:<ul><li>SinglePage Display one page at a
     *                time</li><li>OneColumn Display the pages in one column</li><li>TwoColumnLeft Display the pages in
     *                two columns, with odd-numbered pages on the left</li><li>TwoColumnRight Display the pages in two
     *                columns, with odd-numbered pages on the right</li><li>TwoPageLeft (PDF 1.5) Display the pages two
     *                at a time, with odd-numbered pages on the left</li><li>TwoPageRight (PDF 1.5) Display the pages
     *                two at a time, with odd-numbered pages on the right</li></ul>
     *
     ...
     * Get the canonical page layout mode.
     *
     * @param $mode (string) A name object specifying how the document should be displayed when opened:<ul><li>UseNone
     *              Neither document outline nor thumbnail images visible</li><li>UseOutlines Document outline
     *              visible</li><li>UseThumbs Thumbnail images visible</li><li>FullScreen Full-screen mode, with no
     *              menu bar, window controls, or any other window visible</li><li>UseOC (PDF 1.5) Optional content
     *              group panel visible</li><li>UseAttachments (PDF 1.6) Attachments panel visible</li></ul>
     *
     ...
     *
     * @param $brd      (mixed) Indicates if borders must be drawn around the cell block. The value can be a
     *                  number:<ul><li>0: no border (default)</li><li>1: frame</li></ul>or a string containing some or
     *                  all of the following characters (in any order):<ul><li>L: left</li><li>T: top</li><li>R:
     *                  right</li><li>B: bottom</li></ul> or an array of line styles for each border group:
     *                  array('LTRB' => array('width' => 2, 'cap' => 'butt', 'join' => 'miter', 'dash' => 0, 'color' =>
     *                  array(0, 0, 0)))
     ...
     * @param $diff    (int) If passed, this will be set to the total char number difference between alias and
     *                 replacements.
     *
     ...
     * Reads up to length bytes from the file pointer referenced by handle. Reading stops as soon as one of the
     * following conditions is met: length bytes have been read; EOF (end of file) is reached.
     *
     ...
     *
     * @param $prop        (array) javascript field properties. Possible values are described on official Javascript
     ...
     *                 <li>f or F: Fill the path, using the nonzero winding number rule to determine the region to
     *                 fill.</li>
     *                 <li>f* or F*: Fill the path, using the even-odd rule to determine the region to fill.</li>
     *                 <li>B or FD or DF: Fill and then stroke the path, using the nonzero winding number rule to
     *                 determine the region to fill.</li>
     *                 <li>B* or F*D or DF*: Fill and then stroke the path, using the even-odd rule to determine the
     *                 region to fill.</li>
     *                 <li>b or fd or df: Close, fill, and then stroke the path, using the nonzero winding number rule
     *                 to determine the region to fill.</li>
     *                 <li>b or f*d or df*: Close, fill, and then stroke the path, using the even-odd rule to determine
     *                 the region to fill.</li>
     *                 <li>CNZ: Clipping mode using the even-odd rule to determine which regions lie inside the
     *                 clipping path.</li>
     *                 <li>CEO: Clipping mode using the nonzero winding number rule to determine which regions lie
     *                 inside the clipping path</li>
