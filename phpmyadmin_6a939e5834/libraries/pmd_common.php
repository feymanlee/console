...
function PMA_getTablesInfo()
{
    ...
    foreach ($tables as $one_table) {
        ...
        $GLOBALS['PMD']['TABLE_TYPE'][$i] = /*overload*/
            mb_strtoupper(
                $one_table['ENGINE']
            );
...
function PMA_addNewRelation($db, $T1, $F1, $T2, $F2, $on_delete, $on_update)
{
    ...
    $type_T1 = /*overload*/
        mb_strtoupper($tables[$T1]['ENGINE']);
    $tables  = $GLOBALS['dbi']->getTablesFull($db, $T2);
    $type_T2 = /*overload*/
        mb_strtoupper($tables[$T2]['ENGINE']);
    ...
    if (PMA_Util::isForeignKeySupported($type_T1)
        ...
        if ($foreigner
            ...
            return [false, __('Error: relation already exists.')];
        }
        ...
        if (!empty($index_array1[$F1]) && !empty($index_array2[$F2])) {
            ...
            if ($GLOBALS['dbi']->tryQuery($upd_query)) {
                return [true, __('FOREIGN KEY relation has been added.')];
            } else {
        ...
        if ($GLOBALS['cfgRelation']['relwork'] == false) {
            ...
            if (PMA_queryAsControlUser($q, false, PMA_DatabaseInterface::QUERY_STORE)
                ...
                return [true, __('Internal relation has been added.')];
            } else {
...
function PMA_removeRelation($T1, $F1, $T2, $F2)
{
    ...
    if (PMA_Util::isForeignKeySupported($type_T1)
        ...
        if (isset($foreigner['constraint'])) {
            ...
            if ($GLOBALS['dbi']->query($upd_query)) {
                return [true, __('FOREIGN KEY relation has been removed.')];
            }
    ...
    return [true, __('Internal relation has been removed.')];
}
