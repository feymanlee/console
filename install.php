	...
	<body>
		...
		<div class="warp">
			...
			<form class="form" action="/install.php?action=Install" method="post">
				<fieldset>
					<legend>管理员设置</legend>
					<p><span class="tit">管理员邮箱</span><input type="email" name="bt_email" value="<?php echo $user['email']; ?>" /> *用于找回密码</p>
					<p><span class="tit">用户名</span><input type="text" name="bt_username" value="<?php echo $user['username']; ?>" /> *请设置管理员名称</p>
				...
				<fieldset>
					<legend>路径设置</legend>
					<p><span class="tit">备份文件路径</span><input type="text" name="backup_path" value="<?php echo $config['backup_path']; ?>" /> *站点及数据库打包备份文件的存放路径</p>
					<p><span class="tit">站点创建路径</span><input type="text" name="sites_path" value="<?php echo $config['sites_path']; ?>" /> *站点及FTP创建时的默认路径</p>
				</fieldset>
				<fieldset style="display: none;">
					<legend>MySQL</legend>
					<p><span class="tit">root密码</span><input class='text' name='root' value='<?php echo $config['mysql_root']; ?>'> MySQL数据库管理密码</p>
				</fieldset>
