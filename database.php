...
function AddDatabase(){
	...
	if($data_name == 'root' || $data_name == 'mysql' || $data_name == 'test' || strlen($data_name) < 3){
		ajax_return(array('status'=>false,'msg'=>'数据库名称不合法,或少于3个字符!'));
	}
	...
	if(!preg_match($reg, $data_name)){
		ajax_return(array('status'=>false,'msg'=>'数据库名不能带有特殊符号，且首位必需为字母'));
	}
...
function SetupPassword(){
	...
	if(!is_array($result)){
		...
		if(strpos($result,'using password')){
			...
			ajax_return(array('status'=>false,'msg'=>'root密码不正确，请重新输入!'));
		}
		SqlExec("flush privileges");
		$msg = 'ROOT密码修改成功!';
	}
