...
function oneKeyAdd()
{
    ...
    if (!$data) ajax_return(['status' => false, 'code' => 500, 'msg' => '表单不合法，请重新填写']);
    if (checkMainPath($_POST['path'])) ajax_return(['status' => false, 'code' => 501, 'msg' => '不能以系统关键目录作为站点目录']);

    $MainDomain = explode(":", $data['domain']);
    $reg        = "/^([\w\-\*]{1,100}\.){1,4}(\w{1,10}|\w{1,10}\.\w{1,10})$/";
    if (!preg_match($reg, trim($MainDomain[0]))) ajax_return(['status' => false, 'code' => 501, 'msg' => '主域名格式不正确']);
...
function SetSSL()
{
    ...
    if ($type == 1) {
        ...
        file_put_contents('/tmp/read.tmp', str_replace('(Bt.cn)', '+', urldecode($_POST['key'])));
        SendSocket("FileAdmin|SaveFile|" . $keypath);

        SendSocket('ExecShell|\\cp -a ' . $csrpath . ' /tmp/backup2.conf');
        file_put_contents('/tmp/read.tmp', str_replace('(Bt.cn)', '+', urldecode($_POST['csr'])));
    ...
    SendSocket("FileAdmin|SaveFile|" . $csrpath);
    $data = ['status' => true, 'msg' => '证书已生成!', 'key' => $privatekey, 'csr' => $csrkey];
    serviceWebReload();
    ajax_return($data);
}
...
EOT;
    ...
    if (file_put_contents('/tmp/read.tmp', $conf)) {
        ...
        if (!$sql->where("port='443'")->getCount()) {
            ...
            $data = ['port' => '443', 'ps' => 'https', 'addtime' => date('Y-m-d H:i:s')];
            $sql->add($data);
        }
...
function GetSSL()
{
    ...
    ajax_return(['status' => $status, 'domain' => $domains, 'key' => $key, 'csr' => $csr]);
}
...
function AddSite($own = false)
{
    ...
    $webdir  = str_replace(' ', '', I('post.path'));
...
EOT;
    ...
    //检查处理结果
    if (!$status['status']) {
        $result = ['status' => false, 'code' => -1, 'msg' => (isset($status['msg']) ? $status['msg'] : '连接服务器接口失败!')];
        if ($own) {
            return $result;
        }
        ajax_return($result);
    ...
    if (I('post.ftp') == 'true') {
        ...
        $retuls = ['status' => true, 'in_ftp' => true, 'ftpUserName' => $ftpusername, 'ftpPassword' => $ftppassword, 'ftpUrl' => 'ftp://' . $_SESSION['serverip'] . ':' . $_SESSION['port']];
    } else {
        $retuls = ['status' => true, 'in_ftp' => false];
    }
    //添加数据库
    if (I('post.sql') != 'false') {
        $retuls['sql'] = AddSql();
...
function Get301Status()
{
    ...
    if ($_SESSION['server_type'] == 'nginx') {
        $tmp = file_get_contents('/www/server/nginx/conf/rewrite/' . $siteName . '.conf');
...
function Set301Status()
{
    ...
    if ($_SESSION['server_type'] == 'nginx') {
        if ($srcDomain == 'all') {
            $conf301 = "return 301 $toDomain;";
        } else {
            $conf301 = "if (\$host ~ '^$srcDomain'){\n	return 301 $toDomain\$uri;\n}";
        }
        $conf     = ($type == '1') ? $conf301 : ' ';
        $filename = '/www/server/nginx/conf/rewrite/' . $siteName . '.conf';
    } else {
        $sitePath = M('sites')->where("name='$siteName'")->getField('path');
        if ($srcDomain == 'all') {
            $conf301 = "RewriteEngine on\nRewriteRule ^(.*)\$ $toDomain [L,R=301]";
        } else {
            $conf301 = "RewriteCond %{HTTP_HOST} ^$srcDomain [NC]\nRewriteRule ^(.*)$ $toDomain [L,R=301]";
        }
        $conf     = ($type == '1') ? $conf301 : ' ';
...
function AddSql()
{
    ...
    $retuls = ['dataName' => $data_name, 'dataUser' => $data_name, 'password' => $datapwd];
...
function FilesZip()
{
    ...
    if ($files == '') {
        ajax_return(['status' => false, 'code' => -4, 'msg' => '文件名不能为空']);
    }
    $tmp = explode('.', $files);
    $num = count($tmp);
    if ($num < 2) {
        ajax_return(['status' => false, 'code' => -2, 'msg' => '必需有文件后缀']);
    }
    $exp = strtolower($tmp[$num - 1]);
    if ($exp != 'zip' && $exp != 'gz') {
        ajax_return(['status' => false, 'code' => -3, 'msg' => '当前只支持zip或tar.gz格式的压缩文件']);
    }
    ...
    if ($result['status'] == 'true') {
        ...
        $data = ['status' => true, 'code' => 1, 'msg' => '操作成功'];
    } else {
        $data = ['status' => false, 'code' => -1, 'msg' => $result['msg']];
    }
    ajax_return($data);
}
...
function SetIndex()
{
    ...
    if (!strstr($_GET['Index'], '.')) {
        ajax_return(['status' => false, 'msg' => '输入格式不正确!']);
    };
    ...
    if (strlen($Index) < 3) {
        ajax_return(['status' => false, 'msg' => '默认文档不能为空!']);
    }
    ...
    M('sites')->where("id='$id'")->save(['index' => $Index]);
...
function GetPHPVersion()
{
    ...
    foreach ($phpVersions as $val) {
        if (file_exists('/www/server/php/' . $val . '/bin/php') == true) {
            if ($_SESSION['server_type'] != 'nginx' && $val == '52' && $httpdVersion != '2.2.31') continue;
            $data[$i]['version'] = $val;
            $data[$i]['name']    = 'PHP-' . $val;
            $i++;
        }
...
function SetPHPVersion()
{
    ...
    ajax_return(['status' => false, 'msg' => '保存失败!']);
}
...
function SetDirUserINI()
{
    ...
    file_put_contents($filename, 'open_basedir=' . $path . '/:/tmp/:/proc/');
