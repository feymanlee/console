...
function AddUser()
{
    if (preg_match("/\W+/", $_POST['ftp_username'])) ajax_return(['status' => false, 'code' => 501, 'msg' => '用户名不合法,不能带有特殊符号']);
    if (strlen($_POST['ftp_username']) < 3) ajax_return(['status' => false, 'code' => 501, 'msg' => '用户名不合法,不能少于3个字符']);
    if (checkMainPath($_POST['path'])) ajax_return(['status' => false, 'code' => 501, 'msg' => '不能以系统关键目录作为FTP目录']);
    ...
    if ($ret['status'] == 'true') {
        ...
        ajax_return(['status' => true, 'msg' => '添加失败']);
    }
}
