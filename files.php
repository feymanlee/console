...
function SaveFileBody()
{
    ...
    ajax_return(['status' => false, 'msg' => '转存文件失败!']);
}
...
{
    $dirs = ['/', '/*', '/www', '/root', '/boot', '/bin', '/etc', '/home', '/dev', '/sbin', '/var', '/usr', '/tmp', '/sys',
...
function ExecShell()
{
    $exec = trim(I('exec'));
    $path = trim(I('path'));
    $zs   = ['rm -rf /', 'rm -rf /*', 'rm -rf /boot', 'rm -rf /usr', 'rm -rf /var', 'rm -rf /www'];
...
if (isset($_GET['action'])) {
    $key = ['file', 'dir', 'sfile', 'dfile', 'path', 'user', 'type'];
    foreach ($key as $k) {
        if (isset($_POST[$k])) {
